...
class StyleLossMean(nn.Module):
    """
    Loss class for style matching during style transfer using the mean Gram matrix.
    """
    def __init__(self, target_feature_1, target_feature_2):
        super(StyleLossMean, self).__init__()
        gram1 = gram_matrix(target_feature_1)
        gram2 = gram_matrix(target_feature_2)

        tensor1 = torch.linspace(2, 0, steps=gram1.size(-1), dtype=torch.float, device=gram1.device)
        tensor2 = torch.linspace(0, 2, steps=gram2.size(-1), dtype=torch.float, device=gram2.device)

        tensor1 = tensor1.view(1, -1)
        tensor2 = tensor2.view(1, -1)

        self.target = ((gram1 * tensor1 + gram2 * tensor2) / 2).detach()

    def forward(self, input):
        G = gram_matrix(input)  # Gram matrix of the input
...
def get_style_model_and_losses(cnn, normalization_mean, normalization_std,
    ...
    """
    Build the style transfer model and losses, incorporating StyleLossMean for style blending.
    """
    ...
    for layer in cnn.children():
        ...
        # Style Loss with StyleLossMean
        if name in style_layers:
            target_feature_1 = model(style_img).detach()
            target_feature_2 = model(style_img2).detach()

            style_loss = StyleLossMean(target_feature_1, target_feature_2)
            model.add_module(f"style_loss_{i}_mean", style_loss)
            style_losses.append(style_loss)

    # Truncate the model after the last content or style loss layer
    for i in range(len(model) - 1, -1, -1):
        if isinstance(model[i], ContentLoss) or isinstance(model[i], StyleLossMean):
            break
